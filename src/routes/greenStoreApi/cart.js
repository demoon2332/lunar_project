...
//SAMPLE DATA
const cart_data_sample = require('../../samples/greenStoreApi/cart')
...
router.get('/:uid', (req, res, next) => {
    ...
    Cart.find({ cus_id: uid, oid:""})
        .then(result => {
            ...
            return res.json({ code: 0, message: 'fetch data cart successfully', data: result })
        })
        .catch(err => {
                            ...
                            if (isPID!=null) {
                                Cart.updateOne({ cus_id: uid, oid: "" }, { items: [{
                                    ...
                                    .then(result => {
                                        ...
                                        return res.json({ code: 0, message: 'Add item to order successfully.' })
                                    })
                                    .catch(err => {
                                        return res.json({ code: 100, message: err })
                                    })
                    ...
                    dupPromise.then((isDuplicate) => {
                        if (isDuplicate == 0) {
                            Product.findOne({ pid: pid })
                                .then(result => {
                                    ...
                                    Cart.updateOne({ cus_id: uid, oid: "" }, {
                                        ...
                                        .then(result => {
                                            return res.json({ code: 0, message: "Add item to order successfully. (2)" })
                                        })
                                        .catch(err => {
                            ...
                            Cart.updateOne({ cus_id: uid, oid: "", "items.pid": pid }, {
                                ...
                                .then(result => {
                                    return res.json({ code: 0, message: "Add item to order successfully (1)" })
                                })
                                .catch(err => {
                                    return res.json({ code: 100, message: err })
                                })
                        ...
                        if (isDuplicate == 0) {
                            ...
                            Cart.updateOne({ cus_id: uid, oid: "", "items.pid": pid }, {
                                ...
                                .then(() => {
                                    ...
                                    .then(()=>{
                                        return res.json({ code: 0, message: "Modify item to order successfully."})
                                    })
                                    .catch(err => {
                                        return res.json({ code: 100, message: err })
                                    })
                ...
                if (isPID) {
                    ...
                    dupPromise.then((isDuplicate) => {
                        if (isDuplicate == 0) {
                            ...
                            .then(()=>{
                                return res.json({ code: 0, message: "Delete item in order successfully."})
                            })
                            .catch(err => {
                                return res.json({ code: 100, message: err })
                            })
